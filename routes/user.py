...
async def set_training(training: dict, token: str = Depends(API_Key_Header)) -> JSONResponse:
    async with database.sessions.begin() as session:
        user = await utils.token_to_user(session, token)
        if user is None:
            raise HTTPException(403, {'error': "Пользователь не найден"})
        await session.execute(update(database.Users).where(database.Users.id == user.id).values(current_training = training))
